				...
				<MkFolder>
					...
					<div class="_gaps_s">
						...
						<MkFolder v-if="matchQuery([i18n.ts._role._options.canRemoteNote, 'canRemoteNote'])">
							...
							<template #suffix>{{ policies.canRemoteNote ? i18n.ts.yes : i18n.ts.no }}</template>
							<MkSwitch v-model="policies.canRemoteNote">
								<template #label>{{ i18n.ts.enable }}</template>
							</MkSwitch>
						</MkFolder>
						...
						<MkFolder v-if="matchQuery([i18n.ts._role._options.permissionToPostPublicly, 'permissionToPostPublicly'])">
							...
							<template #suffix>{{ policies.permissionToPostPublicly ? i18n.ts.yes : i18n.ts.no }}</template>
							<MkSwitch v-model="policies.permissionToPostPublicly">
								<template #label>{{ i18n.ts.enable }}</template>
							</MkSwitch>
						</MkFolder>
						...
						<MkFolder v-if="matchQuery([i18n.ts._role._options.canStealNote, 'canStealNote'])">
							...
							<template #suffix>{{ policies.canStealNote ? i18n.ts.yes : i18n.ts.no }}</template>
							<MkSwitch v-model="policies.canStealNote">
								<template #label>{{ i18n.ts.enable }}</template>
							</MkSwitch>
						</MkFolder>
