...
<div class="_gaps">
	...
	<FormSlot>
		...
		<div class="_gaps_s">
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.permissionToPostPublicly, 'permissionToPostPublicly'])">
				<template #label>{{ i18n.ts._role._options.permissionToPostPublicly }}</template>
				<template #suffix>
					<span v-if="role.policies.permissionToPostPublicly.useDefault" :class="$style.useDefaultLabel">{{ i18n.ts._role.useBaseValue }}</span>
					<span v-else>{{ role.policies.permissionToPostPublicly.value ? i18n.ts.yes : i18n.ts.no }}</span>
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.permissionToPostPublicly)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.permissionToPostPublicly.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkSwitch v-model="role.policies.permissionToPostPublicly.value" :disabled="role.policies.permissionToPostPublicly.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts.enable }}</template>
					</MkSwitch>
					<MkRange v-model="role.policies.permissionToPostPublicly.priority" :min="0" :max="2" :step="1" easing :textConverter="(v) => v === 0 ? i18n.ts._role._priority.low : v === 1 ? i18n.ts._role._priority.middle : v === 2 ? i18n.ts._role._priority.high : ''">
						<template #label>{{ i18n.ts._role.priority }}</template>
					</MkRange>
				</div>
			</MkFolder>

			<MkFolder v-if="matchQuery([i18n.ts._role._options.canStealNote, 'canStealNote'])">
				<template #label>{{ i18n.ts._role._options.canStealNote }}</template>
				<template #suffix>
					<span v-if="role.policies.canStealNote.useDefault" :class="$style.useDefaultLabel">{{ i18n.ts._role.useBaseValue }}</span>
					<span v-else>{{ role.policies.canStealNote.value ? i18n.ts.yes : i18n.ts.no }}</span>
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.canStealNote)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.canStealNote.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkSwitch v-model="role.policies.canStealNote.value" :disabled="role.policies.canStealNote.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts.enable }}</template>
					</MkSwitch>
					<MkRange v-model="role.policies.canStealNote.priority" :min="0" :max="2" :step="1" easing :textConverter="(v) => v === 0 ? i18n.ts._role._priority.low : v === 1 ? i18n.ts._role._priority.middle : v === 2 ? i18n.ts._role._priority.high : ''">
						<template #label>{{ i18n.ts._role.priority }}</template>
					</MkRange>
				</div>
			</MkFolder>
...
const save = throttle(100, () => {
	const data = {
		...
		bgColor: role.value.bgColor === '' ? null : role.value.bgColor,
